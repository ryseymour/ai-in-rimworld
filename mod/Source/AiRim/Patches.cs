...
namespace AiRim
{
    ...
    static class Patch_StartJob
    {
        ...
        {
            if (__instance.curJob == newJob) Events.Guard("job_start", () => Events.JobStart(___pawn, newJob));
        }
    }
    ...
    static class Patch_EndCurrentJob
    {
        ...
        {
            Events.Guard("job_end", () => Events.JobEnd(___pawn, __state, condition));
        }
    }
    ...
    static class Patch_MentalState
    {
        ...
        {
            if (__result) Events.Guard("mental_state", () => Events.MentalState(___pawn, stateDef, reason));
        }
    }
    ...
    {
        static void Postfix(Pawn ___pawn, Hediff hediff) => Events.Guard("hediff", () => Events.Hediff(___pawn, hediff));
    }

    [HarmonyPatch(typeof(Pawn), nameof(Pawn.Kill))]
    static class Patch_Kill
    {
        static void Postfix(Pawn __instance, DamageInfo? dinfo) => Events.Guard("death", () => Events.Death(__instance, dinfo));
    }
    ...
    {
        static void Postfix(Letter let) => Events.Guard("letter", () => Events.Letter(let));
    }
    ...
    static class Patch_Incident
    {
        static void Postfix(IncidentWorker __instance, IncidentParms parms, bool __result)
            => Events.Guard("incident", () => Events.Incident(__instance.def, parms, __result));
    }
}
